...
def main():
    ...
    for step in pbar:
        ...
        if step % args.save_step == 0:
            with torch.no_grad():
                latent = encoder(src_data, mode="BB")
                pred_img = decoder(latent)
                emoji_latent = encoder(src_data, mode="AB")
                pred_emoji = decoder(emoji_latent)
                save_image(
                    torch.cat(
                        [(src_data + 1.0) * 0.5, (pred_img + 1.0) * 0.5, (pred_emoji + 1.0) * 0.5],
                        dim=0,
                    ),
